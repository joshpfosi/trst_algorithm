 ...
 *   Last Updated: Sat May 10 21:37:31
 *
 *  calculates speed of boat for given wind directions
 */
...
/*TODO boat needs a speed history, angle_between wind and boat might want to be
  ...
  */
double ideal_speed(Env_data env, Boat_data boat)
{
    Angle angle_to_wind = ang_btwn_angles(env->wind_dir, boat->heading);
    if (angle_to_wind < 30)
        return 0;

    int wa = (angle_to_wind - 30) / 5;
    /* now have a binned wind angle that can be used in polar data */
    double env_wind_speed = env->wind_speed;
    if (env_wind_speed < 6)
        env_wind_speed = 6;
    if (env_wind_speed > 16)
        return polar_data[wa][6];

    int ws = (env_wind_speed - 6) / 2;
